...
import { findUserByToken, getSavedTitles, saveSavedTitles } from '../../_lib/store';
...
export default withJsonHandler(async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (!id || typeof id !== 'string') {
    res.status(400).json({ error: 'Missing saved title id' });
    return;
  }
  ...
  if (index === -1) {
    res.status(404).json({ error: 'Saved title blueprint not found' });
    return;
  }

  const item = saved[index];
  if (user && item.userId !== user.id) {
    res.status(403).json({ error: 'Unauthorized to delete this saved title' });
    return;
  }
  ...
  res.status(200).json({ success: true, message: 'Saved title deleted' });
});
